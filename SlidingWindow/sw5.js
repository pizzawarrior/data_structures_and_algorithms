...
const noRepeats = (str) => {
  ...
  for (let end = 0; end < str.length; end++) {
    ...
    if (rightChar in distinctChars) {
      ...
      // update window start to the greater of the current start index OR the index of the last occurrence of rightChar +1
...
/* Abstract: USE INDEXES NOT COUNTS. create a hashmap to store key (char) and value (index) of each letter as we iterate thru input string. As soon as we encounter a char that is already in the hashmap we set the new window start as the max of start (whatever the current index is) or the index of the last occurence of the char +1
